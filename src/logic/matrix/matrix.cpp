...
// template <class tMatr>
// Matrix<tMatr>::Matrix(const char*& strMatrix){ 
//     uint64_t nStrMatrLen = 0;
//     const char* toEndstrMatrixCopy = strMatrix;
//     while (*strMatrix!='\0'){
//         nStrMatrLen++;
//         toEndstrMatrixCopy++;
//     }
//     std::copy(strMatrix, toEndstrMatrixCopy, std::back_inserter(m_));
// }

template <class tMatr>
Matrix<tMatr>::Matrix(const tMatr& m){ 
    m_ = m;
}

template <class tMatr>
Matrix<tMatr>::Matrix(uint64_t nCols, uint64_t nRows){ 
...
template <class tMatr>
void Matrix<tMatr>::show(){
    uint64_t nCols = m_.size();
    uint64_t nRows = m_[0].size();
    for (uint64_t i = 0; i < nCols; i++){
        for(uint64_t j = 0; j < nRows; j++){
            if (j == 0){
                std::cout << "| ";
            }
            std::cout << m_[i][j] << (j == (nRows-1) ? ' ' : '\t');
        }
        std::cout<<"|\n";
    }
    std::cout << '\n';
